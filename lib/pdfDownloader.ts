...
type TextInput = string | string[] | object | null | undefined;

const cleanMarkdown = (text: TextInput): string => {
  if (!text) return "";
...
const formatTechStack = (
  techStack: TechStack | string | null | undefined
): string => {
...
const formatLandingPage = (
  landingPage: LandingPageContent | null | undefined
): string => {
  ...
  if (landingPage.benefits && Array.isArray(landingPage.benefits)) {
    ...
    landingPage.benefits.forEach((benefit: string, index: number) => {
...
const formatUserPersonas = (
  userPersonas: UserPersona[] | null | undefined
): string => {
