...
export class UserService {
  ...
  static async updatePlanStatus(clerkId, isPremium) {
    try {
      ...
      // Get current user to check existing prompt count
      const currentUser = await User.findOne({ clerkId });
      if (!currentUser) throw new Error("User not found");

      // When upgrading to premium, give them the full limit if they have fewer prompts
      // When downgrading, cap their prompts to the new limit
      let promptsRemaining;
      if (isPremium) {
        // Upgrading: give them at least the new limit
        promptsRemaining = Math.max(
          currentUser.promptsRemaining,
          dailyPromptsLimit
        );
      } else {
        // Downgrading: cap to new limit
