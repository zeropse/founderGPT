...
export class PlanService {
  static async getAllPlans(): Promise<PlanDocument[]> {
    try {
      await connectDB();
  ...
  static async getPlanById(id: string): Promise<PlanDocument> {
    try {
      await connectDB();
      ...
      console.error(`❌ Error fetching plan with id ${id}:`, (error as Error).message);
      throw error;
    }
  }

  static async seedDefaultPlans(): Promise<PlanDocument[]> {
    try {
      await connectDB();
      ...
      console.error("❌ Error seeding default plans:", (error as Error).message);
      throw error;
    }
  }
}
