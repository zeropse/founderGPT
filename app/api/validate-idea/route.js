...
Idea: ${enhancedIdea}`
        ...
        if (techStack) {
          ...
          results.techStack = {
            frontend:
              extractField(techStackLines, "frontend") || "Not specified",
            backend: extractField(techStackLines, "backend") || "Not specified",
            database:
              extractField(techStackLines, "database") || "Not specified",
        ...
        if (landingPage) {
          const landingPageLines = landingPage
            .split("\n")
            .map((line) => line.trim())
            .filter((line) => line !== "");
          results.landingPage = {
            headline:
              extractField(landingPageLines, "headline") || "Not specified",
            subheading:
              extractField(landingPageLines, "subheading") || "Not specified",
        ...
        if (userPersonas) {
          const personaSections = userPersonas
            .split(/\d+\.\s/)
          ...
          results.userPersonas = personaSections.map((section) => {
            ...
            return {
              ...
              painPoints:
                extractPersonaField(lines, "pain points") || "Not specified",
              goals: extractPersonaField(lines, "goals") || "Not specified",
              solution:
                extractPersonaField(lines, "solution") || "Not specified",
            };
          });
        }
      } catch (premiumError) {
        ...
        if (
          premiumError.message.includes("quota") ||
          premiumError.message.includes("rate limit")
        ) {
          return Response.json(
    ...
    return Response.json(
      ...
      {
        status: error.message.includes("quota")
          ? 429
