...
export function SidebarProvider({
  ...
  const chatSelectHandlerRef = useRef<
    ((selectedChat: ChatHistory) => void) | undefined
  >(undefined);
  const chatDeleteHandlerRef = useRef<(() => void) | undefined>(undefined);

  const setSidebarRef = useCallback((ref: React.MutableRefObject<any>) => {
    sidebarRef.current = ref.current;
  }, []);

  const setChatSelectHandler = useCallback(
    (handler: (selectedChat: ChatHistory) => void) => {
